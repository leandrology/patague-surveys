...
import { Box, Flex, Spinner, Text, Container, Stack, Heading, Button, Card, TextInput } from '@sanity/ui';
import shallowEquals from 'shallow-equals';
import isEqual from 'lodash/isEqual.js';
import { SyncIcon, ArrowLeftIcon, SpinnerIcon, SearchIcon } from '@sanity/icons';
import styled, { keyframes } from 'styled-components';
import { defer, mergeMap, throwError, of, share, partition, merge, take, throttleTime, asyncScheduler, Subject, concat, fromEvent, map as map$1, tap, debounce, timer } from 'rxjs';
import { useObservableCallback } from 'react-rx';
import { collate, useSchema, getPublishedId, CommandList, SanityDefaultPreview, createSearchQuery, useClient, DEFAULT_STUDIO_CLIENT_OPTIONS, useSource, useUnique, SourceProvider } from 'sanity';
import 'sanity/router';
import { usePaneLayout, usePane, Delay, PaneContent, useDeskTool, PaneHeader, PaneHeaderActions, BackLink, Pane, _DEBUG } from './desk-0917fc19.js';
import { useDeskToolSetting } from './index-8ed41fa7.js';
import * as PathUtils from '@sanity/util/paths';
import { isKeySegment, isIndexSegment, isReferenceSchemaType } from '@sanity/types';
import { PaneItem } from './PaneItem-0226d52e.js';
import 'lodash/omit.js';
import 'lodash/isNumber.js';
import 'lodash/isString.js';
import { map, startWith, catchError, take as take$1, mergeMap as mergeMap$1, scan } from 'rxjs/operators';
...
const RootBox = styled(Box)(_templateObject || (_templateObject = _taggedTemplateLiteral(["\n  position: relative;\n"])));
const CommandListBox = styled(Box)(_templateObject2 || (_templateObject2 = _taggedTemplateLiteral(["\n  position: absolute;\n  left: 0;\n  right: 0;\n  top: 0;\n  bottom: 0;\n"])));
...
function LoadingView(props) {
  ...
  return /* @__PURE__ */jsx(Stack, {
    padding: 2,
    space: 1,
    children: SKELETON_ITEMS.map(num => /* @__PURE__ */jsx(Card, {
      padding: 2,
...
function DocumentListPaneContent(props) {
  ...
  const renderItem = useCallback((item, _ref) => {
    let {
      activeIndex
    } = _ref;
    ...
    return /* @__PURE__ */jsxs(Fragment, {
      children: [/* @__PURE__ */jsx(PaneItem, {
      ...
      }), showMaxItemsMessage && /* @__PURE__ */jsx(Box, {
        marginY: 1,
        paddingX: 3,
        paddingY: 4,
  ...
  const mainContent = useMemo(() => {
    ...
    if (loadingVariant === "initial" && isLoading) {
      return /* @__PURE__ */jsx(Delay, {
        ms: 300,
        children: /* @__PURE__ */jsx(LoadingView, {
          layout
        })
      });
    }
    ...
    return /* @__PURE__ */jsx(RootBox, {
      overflow: "hidden",
      height: "fill",
      children: /* @__PURE__ */jsx(CommandListBox, {
...
const DocumentListPaneHeader = memo(_ref2 => {
  let {
    contentAfter,
    index,
...
function useDocumentList(opts) {
  ...
  const schema = useSchema();
  const [resultState, setResult] = useState(INITIAL_STATE);
  const {
    onRetry,
    error,
    result
  ...
  const items = useMemo(() => documents ? removePublishedWithDrafts(documents) : EMPTY_ARRAY$1, [documents]);
  const [isLazyLoading, setIsLazyLoading] = useState(false);
  const [hasFullList, setHasFullList] = useState(false);
  const [shouldFetchFullList, setShouldFetchFullList] = useState(false);
  const typeNameFromFilter = useMemo(() => getTypeNameFromSingleTypeFilter(filter, paramsProp), [filter, paramsProp]);
  ...
  const handleSetResult = useCallback(res => {
    var _a, _b;
    if (res.error) {
      setResult(res);
      return;
  ...
  const queryResults$ = useMemo(() => {
    ...
    return listenSearchQuery({
      ...
      return concat(of({
        result: null,
        error: err
      }), merge(fromEvent(window, "online"), onRetry$).pipe(take$1(1), mergeMap$1(() => caught$)));
    }), scan((prev, next) => ({
      ...prev,
      ...next,
      onRetry: _onRetry
...
const EMPTY_ARRAY = [];
const rotate = keyframes(_templateObject3 || (_templateObject3 = _taggedTemplateLiteral(["\n  from {\n    transform: rotate(0deg);\n  }\n  to {\n    transform: rotate(360deg);\n  }\n"])));
const AnimatedSpinnerIcon = styled(SpinnerIcon)(_templateObject4 || (_templateObject4 = _taggedTemplateLiteral(["\n  animation: ", " 500ms linear infinite;\n"])), rotate);
const SearchCard = styled(Card)(_templateObject5 || (_templateObject5 = _taggedTemplateLiteral(["\n  [data-ui='TextInput'] {\n    border-radius: inherit;\n  }\n"])));
...
const DocumentListPane = memo(function DocumentListPane2(props) {
  ...
  const showSearchLoadingRef = useRef(false);
  ...
  const handleQueryChange = useObservableCallback(event$ => {
    return event$.pipe(map$1(event => event.target.value), tap(setSearchInputValue), debounce(value => value === "" ? of("") : timer(300)), tap(setSearchQuery));
  }, []);
  ...
  const searchInput = /* @__PURE__ */jsx(Box, {
    paddingX: 2,
    paddingBottom: 2,
...
//# sourceMappingURL=index-8758d7a4.js.map
